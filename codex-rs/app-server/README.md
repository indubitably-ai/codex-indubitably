...
- `imageGeneration` — `{id, status, revisedPrompt, result, savedPath}` describing a generated image; `savedPath` is `null` until the artifact is saved and is retained in persisted thread history when available.
